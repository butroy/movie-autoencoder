...
class DAE:
    ...
    def init_parameters(self):
        ...
        with tf.name_scope('weights'):
            self.W_1=tf.get_variable(name='weight_1', shape=(self.arguments['num_v'],self.arguments['num_h']), 
                                     initializer=self.weight_initializer)
            self.W_2=tf.get_variable(name='weight_2', shape=(self.arguments['num_h'],self.arguments['num_h']), 
                                     initializer=self.weight_initializer)
            self.W_3=tf.get_variable(name='weight_3', shape=(self.arguments['num_h'],self.arguments['num_h']), 
                                     initializer=self.weight_initializer)
            self.W_4=tf.get_variable(name='weight_4', shape=(self.arguments['num_h'],self.arguments['num_v']), 
                                     initializer=self.weight_initializer)
        
        with tf.name_scope('biases'):
            self.b1=tf.get_variable(name='bias_1', shape=(self.arguments['num_h']), 
                                    initializer=self.bias_initializer)
            self.b2=tf.get_variable(name='bias_2', shape=(self.arguments['num_h']), 
                                    initializer=self.bias_initializer)
            self.b3=tf.get_variable(name='bias_3', shape=(self.arguments['num_h']), 
                                    initializer=self.bias_initializer)
    ...
    def _optimizer(self, x):
        ...
        if self.arguments['l2_reg']==True:
            l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables()])
            MSE_loss = MSE_loss +  self.arguments['lambda_'] * l2_loss
        
        train_op=tf.train.AdamOptimizer(self.arguments['learning_rate']).minimize(MSE_loss)
