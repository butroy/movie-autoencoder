...
fig,ax = plt.subplots()
for key in training_loss_all.keys():
    ax.plot(np.arange(FLAGS.num_epoch),training_loss_all[key],label= 'batch_size:' + str(key))
    ...
    ax.set_ylim((0,2))
plt.plot()
fig.savefig('p1_batch_size_train.png',dpi=500)

fig,ax = plt.subplots()
for key in testing_loss_all.keys():
    ax.plot(np.arange(FLAGS.num_epoch),testing_loss_all[key],label= 'batch_size:' + str(key))
    ax.set_xlabel('num of epoch')
    ax.set_ylabel('RMSE loss')
    ...
    ax.set_ylim((0,1.5))
plt.plot()
fig.savefig('p1_batch_size_test.png',dpi=500)
...
ax.legend()
plt.show()
fig.savefig('p4_l2reg.png',dpi=500)